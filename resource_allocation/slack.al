...
;;//// SLACK BOT INVOKE ON TAGGING ONLY AND MSG FILTERING//////
...
;; ;; Formatting helpers - convert Clojure data to Slack blocks
;; (defn- format-fields
;;   "Turn a map of k/v into Slack section blocks"
;;   [m]
;;   (mapv (fn [[k v]]
;;           {:type "section"
;;            :text {:type "mrkdwn"
...
;; (defn- format-message [msg]
;;   "Convert an agent response into Slack message payload: either blocks or plain text."
;;   (cond
;;     (map? msg)
;;     ;; Single record: section per field
;;     [:blocks (json/encode (format-fields msg))]

;;     (and (sequential? msg) (every? map? msg))
;;     ;; List of maps: header + divider + records
;;     (let [blocks
;;           (cons {:type "header" :text {:type "plain_text" :text "Results"}}
;;                 (interpose {:type "divider"}
;;                            (mapcat format-fields msg)))]
...
;;     ;; register outgoing send function
;;     (reset! send-msg-fn
;;             (fn [reply-msg]
;;               (let [[t payload] (format-message reply-msg)
;;                     resp       (send-message-to-channel ch-id payload t)
;;                     ts         (extract-ts resp)]
;;                 (reset! last-sent-ts ts)
;;                 ts)))
;;     ;; send welcome message once
;;     (let [init-ts (extract-ts
;;                    (send-message-to-channel ch-id
;;                                             (make-welcome-message doc schema-doc)))]
;;       (future
;;         (loop [last-ts init-ts]
;;           (Thread/sleep 10000)
;;           (when (get @run-flags name)
...
;; Formatting helpers - convert Clojure data to Slack blocks, filtering out metadata fields
(defn- format-fields
  "Turn a map of k/v into Slack section blocks, excluding internal keys."
  [m]
  (->> m
       (remove (fn [[k _]]
                 (let [n (name k)]
                   (or (s/starts-with? n "__")
                       (s/starts-with? n "-")
                       (s/includes? n "*")
                       (s/includes? n ":")))))
       (mapv (fn [[k v]]
...
(defn- format-message [msg]
  "Convert an agent response into Slack blocks or plain text."
  (cond
    (map? msg)
    [:blocks (json/encode (format-fields msg))]
